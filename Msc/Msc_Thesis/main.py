...
class Main:
    def __init__(self, config):
        ...
        is_logging = not self.config["is_iterative_train_mdrnn"] and (self.config["is_train_vae"] or self.config["is_train_mdrnn"])
        self.logger = TensorboardHandler(is_logging)
    ...
    def train_or_reload_models(self):
        if self.config["is_generate_data"]:
            self.generate_data()
        ...
        if config["is_iterative_train_mdrnn"] and not config["is_train_mdrnn"]:
            mdrnn = self.mdrnn_trainer.reload_model(mdrnn)
            iterative_trainer = IterativeTrainer(config, agent, main.mdrnn_trainer)
            iterative_trainer.train()

        if self.config["is_train_mdrnn"]:
...
if __name__ == '__main__':
    ...
    if config['is_ntbea_param_tune']:
        agent = main.get_planning_agent(config['planning']['planning_agent'])  # Get agent
