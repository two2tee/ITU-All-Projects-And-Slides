...
public class TestLiftGui {
  public static void main(String[] args) {
    ...
    Thread t1 = new Thread(lift1), t2 = new Thread(lift2),
            t3 = new Thread(lift3), t4 = new Thread(lift4);
    final long steps = 16;
    ScheduledThreadPoolExecutor threadPool = new ScheduledThreadPoolExecutor(8);
    threadPool.scheduleAtFixedRate(t1, 0, (long)1000.0/steps, TimeUnit.MILLISECONDS);
    threadPool.scheduleAtFixedRate(t2, 0, (long)1000.0/steps, TimeUnit.MILLISECONDS);
    threadPool.scheduleAtFixedRate(t3, 0, (long)1000.0/steps, TimeUnit.MILLISECONDS);
    threadPool.scheduleAtFixedRate(t4, 0, (long)1000.0/steps, TimeUnit.MILLISECONDS);
    ...
    left.add(lift1Display, BorderLayout.WEST);
    left.add(lift2Display, BorderLayout.EAST);

    right.add(lift3Display, BorderLayout.WEST);
    right.add(lift4Display, BorderLayout.EAST);
...
class InsideLiftButtons extends JPanel {
  public InsideLiftButtons(final Lift lift) {
    ...
    for (int floor=lift.highFloor; lift.lowFloor<=floor; floor--) {
      ...
      lift.setFloorButton(button, floor-lift.lowFloor);
      panel.add(button);
...
class Lift implements Runnable {
  ...
  public Lift(String name, LiftShaft shaft) {
    ...
    this.floorButtons = new JButton[highFloor-lowFloor+1];
  }

  // All these private methods are used in the lift's
  // up-sweep/down-sweep operation:
  public synchronized void setFloorButton(JButton floorButton, int floor){
    System.out.println(floorButtons.length);
    System.out.println(floor);
    floorButtons[floor] = floorButton;
  }
  ...
  private synchronized void subtractFromStop(int floor, Direction dir) {
    floorButtons[floor-lowFloor].setBackground(new JButton().getBackground());
    switch (dir) {
      case Down:
        if (floor == lowestStop())
          setStop(floor, null);
