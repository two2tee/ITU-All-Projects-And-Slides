...
public class TestFetchWebGui {
  ...
  private static void goodFetch() {
    ...
    for (String url: urls){
      ConcurrentWorker downloadTask = new ConcurrentWorker(textArea,url);
      fetchButton.addActionListener(e -> downloadTask.execute());
      cancelButton.addActionListener(e -> downloadTask.cancel(false));
      downloadTask.addPropertyChangeListener(e ->
      {
  ...
  static class ConcurrentWorker extends SwingWorker<String,String>
  {
    private final TextArea textArea;
    private final String url;

    public ConcurrentWorker(TextArea textArea, String url) {
      this.textArea = textArea;
      this.url = url;
    }
    // Called on a separate worker thread, not the event thread, and
    ...
    public String doInBackground() {
        ...
        if (isCancelled()) {                // (3)
          return "Cancelled fetching of url: "+url;
        }
        System.out.println("Fetching " + url);
        String page = getPage(url, 200);
    ...
    public void done() {
      ...
      catch (CancellationException exn) { textArea.append("Cancelled fetch of " + url "\n"); }  // (3)
    }
  }
}
